...
#print an informative message announcing the vote 
    if choice == "yes":
      print("%s voted YES on %s bill" % (self.name, bill.title))
    elif choice == "no":
      print("%s voted NO on %s bill" % (self.name, bill.title))
    elif choice == "abstain":
      print("%s ABSTAINED on %s bill" % (self.name, bill.title))
    else:
      print("no record")
...
class Bill():
  ...
  def result(self):
    ...
    if len(self.votes["yes"]) > len(self.votes["no"]):
      self.passed = True
      print("%s bill PASSED" % self.title)
    else:
      print("%s bill did NOT pass" % self.title)
      self.passed = False
...
mary = Senator("Mary")
mary.vote(environment, "yes")
environment.result()
print(environment.votes)
print(environment.passed)

banking = Bill("Banking Regulation")
jane.vote(banking, "yes")
jack.vote(banking, "yes")
mary.vote(banking, "yes")
banking.result()
